...
const gradientMapping: Record<string, string> = {
    ...
    cyan: 'linear-gradient(hsl(183, 90%, 40%), hsl(168, 90%, 40%))',
    white: 'linear-gradient(hsl(0, 0%, 90%), hsl(0, 0%, 70%))',
};
        ...
        <div className={`grid gap-[5em] grid-cols-2 md:grid-cols-3 mx-auto py-[3em] overflow-visible ${className || ''}`}>
            {items.map((item, index) => (
                <button
                    ...
                    className={`relative bg-transparent outline-none border-none cursor-pointer w-[6em] h-[6em] [perspective:24em] [transform-style:preserve-3d] [-webkit-tap-highlight-color:transparent] group ${item.customClass || ''
                        }`}
                >
                    <span
                    ...
                    <span className="absolute top-full left-0 right-0 text-center whitespace-nowrap leading-[2.5] text-lg opacity-0 transition-[opacity,transform] duration-300 ease-[cubic-bezier(0.83,0,0.17,1)] translate-y-0 group-hover:opacity-100 group-hover:[transform:translateY(20%)]">
                        {item.label}
                    </span>
                </button>
            ))}
