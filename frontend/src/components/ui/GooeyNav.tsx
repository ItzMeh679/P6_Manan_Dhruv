...
const GooeyNav: React.FC<GooeyNavProps> = ({ items, animationTime = 600, particleCount = 15, particleDistances = [90, 10], particleR = 100, timeVariance = 300, colors = [1, 2, 3, 1, 2, 3, 1, 4], initialActiveIndex = 0, onItemClick }) => {
    ...
    const handleClick = (e: React.MouseEvent<HTMLAnchorElement>, index: number) => {
        ...
        if (textRef.current) { textRef.current.classList.remove('active'); void (textRef.current as HTMLElement).offsetWidth; textRef.current.classList.add('active'); }
